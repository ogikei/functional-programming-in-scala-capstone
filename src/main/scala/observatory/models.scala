...
case class Tile(x: Double, y: Double, zoom: Int) {
  ...
  def toURI =
    new java.net.URI("http://tile.openstreetmap.org/" + zoom + "/" + x + "/" + y + ".png")
}
...
case class Point(ϕ: Double, λ: Double) {
   ...
   * https://www.coursera.org/learn/scala-capstone/discussions/weeks/2/threads/YY0u6Ax8EeeqzRJFs29uDA
   *
...
case class StationDate(day: Int, month: Int, year: Int) {
  def toLocaleDate: LocalDate = LocalDate.of(year, month, day)
}
